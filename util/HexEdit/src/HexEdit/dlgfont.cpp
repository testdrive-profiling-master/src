...
static PVOID CopyDialogItem(DLGITEMTEMPLATEEX *destDlg, PVOID source, BOOL fExtended)
{
	...
	if(fExtended)
	{
		DLGITEMTEMPLATEEX *srcDlg = (DLGITEMTEMPLATEEX*)source;
	...
	{
		DLGITEMTEMPLATE *srcDlg = (DLGITEMTEMPLATE*)source;
	...
	sptr = (WCHAR*)RoundUp(sptr, sizeof(DWORD));
...
static PVOID CopyDialog(DLGTEMPLATEEX *destDlg, PVOID source, BOOL fExtended, WCHAR *szNewFontName, WORD nPointSize)
{
	...
	if(fExtended)
	{
		DLGTEMPLATEEX *srcDlg = (DLGTEMPLATEEX*)source;
	...
	{
		DLGTEMPLATE *srcDlg = (DLGTEMPLATE*)source;
...
HGLOBAL DialogExFromTemplate(HINSTANCE hInst, HANDLE hTemplate, LPCTSTR szFontName, WORD nPointSize)
{
	...
	if((srcDlg = (DLGTEMPLATEEX*)GlobalLock(hTemplate)) == 0)
		return NULL;
	...
	//
	if((destDlg = (DLGTEMPLATEEX*)GlobalAlloc(GPTR, (DWORD)srcItem - (DWORD)srcDlg + (lstrlenW(wszFontName) + 1) * sizeof(WCHAR))) == 0)
	{
	...
	for(i = 0; i < count; i++)
	{
		srcItem  = CopyDialogItem((DLGITEMTEMPLATEEX*)destItem, srcItem,  fExtended);
...
BOOL DialogBoxWithFont(HINSTANCE hInst, LPCTSTR lpTemplateName, HWND hwndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam, LPCTSTR szFontName, WORD nPointSize)
{
	...
	retCode = (BOOL)DialogBoxIndirectParam(hInst, (LPCDLGTEMPLATEW)hTemplate, hwndParent, lpDialogFunc, dwInitParam);
...
HWND CreateDialogWithFont(HINSTANCE hInst, LPCTSTR lpTemplateName, HWND hwndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam, LPCTSTR szFontName, WORD nPointSize)
{
	...
	hwndDialog = CreateDialogIndirectParam(hInst, (LPCDLGTEMPLATEW)hTemplate, hwndParent, lpDialogFunc, dwInitParam);
