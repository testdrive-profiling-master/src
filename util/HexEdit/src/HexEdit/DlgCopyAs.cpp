...
INT_PTR CALLBACK CopyAsDlgProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
		case IDOK:
			g_CopyOptions.format     = (IMPEXP_FORMAT)(SendDlgItemMessage(hwnd, IDC_EXPORTFORMAT, CB_GETCURSEL, 0, 0) + 1);
			g_CopyOptions.basetype   = (SEARCHTYPE)ComboBox_GetSelData(GetDlgItem(hwnd, IDC_DATATYPE));
