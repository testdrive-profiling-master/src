...
INT_PTR CALLBACK ExportHookProc(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		...
		{
			g_ExportOptions.format		= (IMPEXP_FORMAT)(ofn->lpOFN->nFilterIndex - 1);
			...
			g_ExportOptions.basetype	= (SEARCHTYPE)ComboBox_GetSelData(GetDlgItem(hdlg, IDC_DATATYPE));
		}
...
BOOL ExportMotorola(FILE *fp, HWND hwndHexView, size_w offset, size_w length, IMPEXP_OPTIONS *eopt)
{
	...
	// frame header (type=0)
	alen = motorola_frame(ach, 0, 3, 0, (BYTE*)"HDR");
	fprintf(fp, "%.*s\n", alen, ach);
