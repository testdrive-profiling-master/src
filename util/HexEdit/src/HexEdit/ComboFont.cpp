...
void MakeFontCombo(HWND hwndDlg, HWND hwndCombo)
{
	...
	//
	g_hIcon2 = (HICON)LoadImage(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON4), IMAGE_ICON, 16, 16, 0);
	g_hIcon3 = (HICON)LoadImage(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON4), IMAGE_ICON, 16, 16, 0);
	...
	cip = SubclassCombo(hwndCombo, (DrawItemProc)FontCombo_DrawItem);

	// fill the combo with the list of all fonts
	FillFontComboList(hwndCombo, TEXT("Arial"));//g_szFontName);
	
	AutoOwnerDraw(cip->hTheme, hwndCombo, (DrawItemProc)FontCombo_DrawItem, 0);
}
