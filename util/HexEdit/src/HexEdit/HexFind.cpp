...
extern "C" BOOL UpdateSearchDataEx(HWND hwndSource, SEARCHTYPE searchType, BOOL fBigEndian, BYTE *searchData, int *searchLen);
...
BOOL Text2Binary(WCHAR *szText, BYTE *buf, int *buflen, int codepage, BOOL fBigEndian)
{
	...
	{
		len = WideCharToMultiByte(codepage, 0, szText, -1, (LPSTR)buf, len, 0, 0);
...
BOOL UpdateSearchDataEx(HWND hwndSource, SEARCHTYPE searchType, BOOL fBigEndian, BYTE *searchData, int *searchLen)
{
...
BOOL UpdateSearchDataDlg(HWND hwndDlg, int sourceId, BOOL fBigEndian, BYTE *searchData, int *searchLen)
{
	...
	if(UpdateSearchDataEx(GetDlgItem(hwndDlg, sourceId), (SEARCHTYPE)searchType, fBigEndian, searchData, searchLen))
	{
...
BOOL FindNext()
{
	...
	else
	{
		...
		MessageBox(g_hwndMain, TEXT("Could not find any more data"), TEXT("HexEdit for TestDrive"), MB_OK|MB_ICONEXCLAMATION);
		return FALSE;
	}
}
...
BOOL Find(HWND hwnd, HWND hwndHV)
{
	...
	GetDlgItemText(hwnd, IDC_COMBO1, szText, MAX_SEARCH_TEXT);
			...
			searchValid = UpdateSearchDataDlg(hwnd, IDC_COMBO1, g_fBigEndian, searchData, (int*)&searchLen);
...
BOOL Replace(HWND hwndDlg, HWND hwndHV)
{
	...
	UpdateSearchDataEx(GetDlgItem(hwndDlg, IDC_COMBO2), (SEARCHTYPE)searchType, g_fBigEndian, replaceData, (int*)&replaceLen);
...
//	TCHAR szText[MAX_SEARCH_TEXT];
	...
	case WM_COMMAND:
		...
		if(HIWORD(wParam) == CBN_EDITUPDATE || HIWORD(wParam) == CBN_SELCHANGE)
		{
			...
			{
				replaceLen   = sizeof(replaceData);
				replaceValid = UpdateSearchDataDlg(hwnd, IDC_COMBO2, g_fBigEndian, replaceData, (int*)&replaceLen);
