...
BOOL HexSetCurFile(HWND hwndMain, int iItem, BOOL fSetFocus)
{
	...
	if(!name) name = _tcsrchr(szFilePath, '/');
...
BOOL HexOpenFile(HWND hwndMain, LPCTSTR szFileName, DWORD fHexViewFlags)
{
	...
	if(HexView_OpenFile(hwndHV, szFileName, fHexViewFlags))
	{
		...
		if(e == ERROR_SHARING_VIOLATION)
		{
			...
			//MessageBox(hwndMain, TEXT("Another "), TEXT("HexEdit for TestDrive"), MB_ICONINFORMATION);
		}
...
{
	if(szFile && _tcsstr(szFile, _T("MMAP://")) == szFile){
		_tcscpy(g_szFileName, szFile);
		_tcscpy(g_szFileTitle, szFile+7);
	}else{
		TCHAR *name;
		TCHAR *fp;
