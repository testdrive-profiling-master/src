...
bool sequence::buffer_control::load(const TCHAR * filename, size_w* pBaseAddress, bool reado, bool quickload)
{
	DWORD dwAccess = GENERIC_READ;
	DWORD dwError  = S_OK;
	bool  success  = false;
	...
	if(_tcsstr(filename, _T("MMAP://")) == filename){
		LPCTSTR sDelim	= _T("/\\");
		LPCTSTR	sTok;
		TCHAR	sTag[1024], sName[MAX_PATH];

		_tcscpy(sTag, filename+7);
		sTok	= _tcstok(sTag, sDelim);
		if(sTok){
			_tcscpy(sName, sTok);
			sTok	= _tcstok(NULL, sDelim);
		}
		if(sTok){
			_stscanf(sTok, _T("%X"), &mmap.dwBaseAddress);
			sTok	= _tcstok(NULL, sDelim);
		}
		if(sTok){
			_stscanf(sTok, _T("%X"), &mmap.dwViewOffset);
			mmap.dwBaseAddress	+= mmap.dwViewOffset;
			sTok	= _tcstok(NULL, sDelim);
		}
		if(sTok){
			_stscanf(sTok, _T("%X"), &mmap.dwViewSize);
			sTok	= _tcstok(NULL, sDelim);
		}

		if((mmap.pMem = TestDriver_GetMemory(sName))){
			DWORD	dwSize	= mmap.pMem->GetSize() - mmap.dwViewOffset;
			BYTE*	pBuff	= mmap.pMem->GetPointer(mmap.dwViewOffset, mmap.dwViewSize);

			if(pBaseAddress) *pBaseAddress	= mmap.dwBaseAddress;

			if(!pBuff) goto EXIT_LOAD;

			if(mmap.dwViewSize)
				dwSize	= mmap.dwViewSize;

			init(pBuff, dwSize, false);
...
bool sequence::buffer_control::is_mmap(void){
	return mmap.pMem != NULL;
}
