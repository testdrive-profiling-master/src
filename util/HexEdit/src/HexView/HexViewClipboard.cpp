...
BOOL HexView::OnPaste()
{
	...
	if(	m_nEditMode == HVMODE_READONLY ||
		(m_nEditMode == HVMODE_INSERT && m_pDataSeq->is_mmap()))
		return FALSE;
...
BOOL HexView::OnCut()
{
	...
	if(m_nEditMode != HVMODE_INSERT || m_pDataSeq->is_mmap())
		return FALSE;
	...
	if(S_OK == OleIsCurrentClipboard(m_pLastDataObject))
	{
		...
		if(GetDataObjDword(m_pLastDataObject, CFSTR_HEX_DATALEN, &len) == S_OK)
		{
			...
			if(len > 1024)
			{
				...
				answer = MessageBox(m_hWnd, szMessage, _T("HexEdit for TestDrive"), MB_YESNO | MB_ICONQUESTION);
