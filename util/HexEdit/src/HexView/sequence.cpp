...
sequence::span * sequence::loadspan(const TCHAR *filename, bool readonly, bool quickload)
{
	...
	if(!bc->load(filename, NULL, readonly, quickload))
	{
		delete bc;
		return false;
	}
...
//
bool sequence::open(const TCHAR *filename, size_w* pBaseAddress, bool readonly, bool quickload)
{
	if(!clear())
		return false;
	...
	if(bc->load(filename, pBaseAddress, readonly, quickload))
	{
		bc->id = buffer_list.size();		// assign the id
		buffer_list.push_back(bc);
...
bool sequence::writespan(buffer_control* original_bc, size_w index, span *sptr){
	size_w copylen   = sptr->length;
	size_w srcoff	 = sptr->offset;
	...
	while(copylen > 0)
	{
		...
		if((src = bc->getptr(srcoff, len)) == 0)
			return false;
		if((dst = original_bc->getptr(index, len)) == 0)
			return false;
...
bool sequence::saveto(bool quicksave)
{	
	span *sptr;
	size_w spanindex;
	...
	for( ; sptr->next; sptr = sptr->next)
	{
		...
		if(quicksave == false || bc->id != origfile_id)
		{
			if(writespan(buffer_list[origfile_id], spanindex, sptr) == false)
			{
				return false;
			}
		}
...
bool sequence::save(const TCHAR * filename /* = 0*/)
{
	...
	if(origfile_id == -1) return false;

	if(buffer_list[origfile_id]->is_mmap() && can_quicksave)
	{
		TCHAR origfile[MAX_PATH];
		_tcscpy_s(origfile, MAX_PATH, origfile_name);
		saveto(quicksave);
		clear();
		return open(origfile, NULL, false);
		//MessageBox(NULL, origfile, _T("A"), MB_OK);
		return true;
	}
...
sequence::buffer_control* sequence::get_orignal_buffer(void){
	return buffer_list[origfile_id];
}

bool sequence::is_mmap(void){
	buffer_control* bc = get_orignal_buffer();
	if(!bc) return false;
	return bc->is_mmap();
}
//
