...
// MMAP 으로 열기 실행시 5초마다 드로잉 갱신
static VOID CALLBACK __RefreshViewProc__(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime){
	InvalidateRect(hwnd, NULL, FALSE);
	//HexView_SetRedraw(hwnd, TRUE);
}
...
BOOL HexView::OpenFile(LPCTSTR szFileName, UINT uOpenFlags)
{
	...
	if(m_pDataSeq->open(szFileName, &m_nAddressOffset, fReadonly, fQuickload))
	{
		TCHAR *fp;
		DWORD e = GetLastError();
		if(m_pDataSeq->is_mmap()){
			_tcscpy(m_szFilePath, szFileName);
		}else{
		...
		m_nSearchLen = 0;
		if (m_pDataSeq->is_mmap())
			SetTimer(m_hWnd, 0, 5000, __RefreshViewProc__);
		return TRUE;
	}
