...

{
	...
	m_nControlStyles	= HVS_RESIZEBAR|HVS_ALWAYSVSCROLL;	// styles cleared by default
...
void HexView::ShowPhysicalAddress(BOOL bEnable)
{
	g_bViewPhysicalAddress	= bEnable;
	UpdateMetrics();
	RefreshWindow();
	NotifyParent(HVN_CURSOR_CHANGE);
}

void HexView::RetrieveAddress(UINT64& iAddress){
	if(g_bViewPhysicalAddress)
		iAddress	+= m_nAddressOffset;
}
...
bool HexView::Refresh(){
	if(m_pDataSeq->is_mmap()){
		RefreshWindow();
	}
	return TRUE;
}
...
LRESULT HexView::WndProc(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case HVM_SHOWPHYSICAL:
		ShowPhysicalAddress((BOOL)wParam);
		return 0;
	case HVM_RETRIEVE_ADDRESS:
		RetrieveAddress(*((UINT64*)wParam));
		return 0;
