...
public:
	...
	//
	bool		init();
	bool		open(const TCHAR *filename, size_w* pBaseAddress = NULL, bool readonly = false, bool quickload = false);
...
private:
	...
	bool			saveto(bool quicksave);	// to mmap
	bool			writespan(HANDLE hFile, size_w index, span *sptr);
	bool			writespan(buffer_control* original_bc, size_w index, span *sptr);	// to mmap
	buffer_control *get_orignal_buffer(void);
...
public:
	...
	bool		load(const TCHAR * filename, size_w* pBaseAddress, bool reado, bool quickload);
	void		free();
	bool		is_mmap(void);

	seqchar *	getptr(size_w off, size_w len);
	size_w		base_address(void);
