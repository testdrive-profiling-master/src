...
ViewComponent::ViewComponent(
    ...
    : m_pHtml(pHtml), m_controller(pHtml->GetWebViewController()),
    m_webView(pHtml->GetWebView()), m_dcompDevice(dcompDevice),
#ifdef USE_WEBVIEW2_WIN10
...
bool ViewComponent::OnMouseMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
...
#endif
    {
        ...
        if (message == WM_MOUSEWHEEL || message == WM_MOUSEHWHEEL)
        {
            ...
            ::ScreenToClient(m_pHtml->GetMainWindow(), &point);
        }
        ...
        if (isMouseInWebView || message == WM_MOUSELEAVE || m_isCapturingMouse)
        {
            ...
            {
                if (isMouseInWebView && ::GetCapture() != m_pHtml->GetMainWindow())
                {
                    m_isCapturingMouse = true;
                    ::SetCapture(m_pHtml->GetMainWindow());
                }
            }
            else if (message == WM_LBUTTONUP || message == WM_MBUTTONUP ||
                message == WM_RBUTTONUP || message == WM_XBUTTONUP)
            {
                if (::GetCapture() == m_pHtml->GetMainWindow())
                {
...
void ViewComponent::TrackMouseEvents(DWORD mouseTrackingFlags)
{
    ...
    tme.hwndTrack = m_pHtml->GetMainWindow();
    tme.dwHoverTime = 0;
    ::TrackMouseEvent(&tme);
}
...
void ViewComponent::BuildDCompTreeUsingVisual()
{
    ...
    {
        m_dcompDevice->CreateTargetForHwnd(m_pHtml->GetMainWindow(), TRUE, &m_dcompHwndTarget);
