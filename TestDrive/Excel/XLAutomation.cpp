...
BOOL CXLAutomation::SetExcelVisible(BOOL bVisible)
{
	...
	ClearAllArgs();
	return ExlInvoke(m_pdispExcelApp, L"Hide", NULL, DISPATCH_PROPERTYPUT, DISP_FREEARGS);
}
...
int CXLAutomation::GetWorksheetsCount(void)
{
	if(NULL == m_pdispWorkbook) return -1;

	VARIANTARG varg1, varg2;
	ClearAllArgs();
	if (!ExlInvoke(m_pdispWorkbook, L"Worksheets", &varg1, DISPATCH_PROPERTYGET, 0)) return -1;
	ClearAllArgs();
	if (!ExlInvoke(varg1.pdispVal, L"Count", &varg2, DISPATCH_PROPERTYGET, 0)) 	return -1;
...
BOOL CXLAutomation::SetActiveWorksheet(int nWorksheet)
{
	...
	if((nWorksheet < 1) || (nWorksheet > nCount)) return FALSE;
...
BOOL CXLAutomation::SetActiveWorksheet(CString szName){
	int nCount = GetWorksheetsCount();
	...
	for(int i=1;i<=nCount;i++){
		...
		Type = varg2.vt;
		if(Type == VT_BSTR){
			CString str = varg2.bstrVal;
			if(!szName.Compare(str)){
				ClearAllArgs();
...
int CXLAutomation::CountDataCells(int nResultColumn)
{
	...
	if (!ExlInvoke(vargRng.pdispVal, L"FormulaR1C1", NULL, DISPATCH_PROPERTYPUT, 0))
		return -1;
	ReleaseVariant(&vargRng);

	CString szValue;
	GetCellValueCString(nResultColumn,1, &szValue);
	...
	int nDataCells = 65536-_ttoi(szValue);
...
BOOL CXLAutomation::PasteStringToWorksheet(CString* szRange, CString* pDataBuffer)
{
	if(NULL == m_pdispWorksheet)
		return FALSE;
	...
	HANDLE hMem = GlobalAlloc(GMEM_MOVEABLE|GMEM_DDESHARE, 24);
	  ...
	  if (OpenClipboard(NULL) )
	  {
		...
		//WCHAR szRangeW[MAX_PATH];
		//MultiByteToWideChar(CP_ACP, 0, *szRange, szRange->GetLength()+1, szRangeW, MAX_PATH);

		ClearAllArgs();
		//AddArgumentOLEString(NULL, 0, szRangeW);
		AddArgumentOLEString(NULL, 0, szRange->GetBuffer());
		//AddArgumentOLEString(NULL, 0, L"B1");
...
BOOL CXLAutomation::SaveAs(CString szFileName, int nFileFormat, CString szPassword, CString szWritePassword, BOOL bReadOnly, BOOL bBackUp)
{
	...
	// disable 
	ClearAllArgs(); AddArgumentBool(NULL, 0, TRUE); 
	if (!ExlInvoke(m_pdispExcelApp, L"DisplayAlerts", NULL, DISPATCH_PROPERTYPUT, DISP_FREEARGS)) return FALSE; 
...
BOOL CXLAutomation::GetCellValueCString(int nColumn, int nRow, CString* pszValue)
{
	if(NULL == m_pdispWorksheet || !pszValue) return FALSE;
	...
	if(!ExlInvoke(vargRng.pdispVal, L"Value", &vargValue, DISPATCH_PROPERTYGET, 0)) return FALSE;
	...
	switch (Type) 
		{
			case VT_UI1:
				{
					...
					pszValue->Format(_T("%c"), nChr);
				}
				break;
			case VT_I4:
				{
					long nVal = vargValue.lVal;
					pszValue->Format(_T("%i"), nVal);
				}
				break;
			case VT_R4:
				{
					float fVal = vargValue.fltVal;
					pszValue->Format(_T("%f"), fVal);
				}
				break;
			case VT_R8:
				{
					double dVal = vargValue.dblVal;
					pszValue->Format(_T("%f"), dVal);
				}
				break;
			case VT_BSTR:
				{
			...
			/*case VT_BYREF|VT_UI1:
				{
					//Not tested
					TCHAR* pChr = vargValue.pbVal;
					//szValue = *pChr;
					pszValue->Format(_T("%c"), *pChr);
				}
				break;*/
...
DWORD CXLAutomation::GetCellValueRef(int nColumn, int nRow){
	/*if(NULL == m_pdispWorksheet) return 0;
	...
	AddArgumentDouble(NULL, 0, nColumn);
	AddArgumentDouble(NULL, 0, nRow);
	if(!ExlInvoke(m_pdispWorksheet, L"Cells",&vargRng, DISPATCH_PROPERTYGET, DISP_FREEARGS)) return 0;
	if(!ExlInvoke(vargRng.pdispVal, L"Value", &vargValue, DISPATCH_PROPERTYGET, 0)) return 0;
	...
	//	ReleaseVariant(&vargValue);*/
	CString sValue;
	GetCellValueCString(nColumn, nRow, &sValue);
	return _ttoi(sValue);
}

long CXLAutomation::GetCellValueLong(int nColumn, int nRow){
	if(NULL == m_pdispWorksheet) return 0;
	...
	case VT_I4:	// integer
		{
			//long nVal = vargValue.lVal;
			return vargValue.lVal;
			//pszValue->Format("%i", nVal);
		}
		break;
	case VT_R4:	// float
		{
			return (long)vargValue.fltVal;
		}
		break;
	case VT_R8:	// double
		{
			return (long)vargValue.dblVal;
		}
		break;
	}
	return 0;
...
double CXLAutomation::GetCellValueDouble(int nColumn, int nRow){
	if(NULL == m_pdispWorksheet) return 0;
	...
	{
	case VT_I4:	// integer
		return (double)vargValue.lVal;
		break;
	case VT_R4:	// float
		return (double)vargValue.fltVal;
		break;
	case VT_R8:	// double
		return vargValue.dblVal;
		break;
	}
	return 0;
}

DWORD CXLAutomation::GetCellValueHex(int nColumn, int nRow){
	if(NULL == m_pdispWorksheet) return FALSE;
	...
	_tscanf(sValue, _T("0x%x"), &dwValue);
