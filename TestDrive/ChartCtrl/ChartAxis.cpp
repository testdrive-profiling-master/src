...
CChartAxis::CChartAxis() 
 : m_pParentCtrl(NULL), m_bIsHorizontal(true), m_bIsInverted(false), 
   ...
   m_nFontSize(80), m_strFontName(_T("Microsoft Sans Serif")),
   m_bAutoMargin(true), m_iMarginSize(0), m_bZoomEnabled(true), 
   m_dZoomLimit(0.001), m_pScrollBar(NULL)
{
...
typedef enum{
	CMD_AXIS_SET_MINMAX,
	CMD_AXIS_SET_ZOOMLIMIT,
	CMD_AXIS_SET_AUTOMATIC,
	CMD_AXIS_SET_AXISCOLOR,
	CMD_AXIS_SET_TEXTCOLOR,
	CMD_AXIS_SET_INVERTED,
	CMD_AXIS_LABEL,
	CMD_AXIS_GRID,
	CMD_AXIS_SIZE,
}CMD_AXIS;

static const TCHAR* g_sChartAxis[CMD_AXIS_SIZE]={
	_T("SetMinMax"),
	_T("SetZoomLimit"),
	_T("SetAutomatic"),
	_T("SetAxisColor"),
...
BOOL CChartAxis::Paser(LPCTSTR pCmd, CPaser* pPaser){
	if(!pCmd || !pPaser) return FALSE;
	int id = CheckCommand(pCmd, g_sChartAxis, CMD_AXIS_SIZE);
	if(id >= CMD_AXIS_SIZE) return FALSE;

	if(!pPaser->TokenOut(id < CMD_AXIS_LABEL ? TD_DELIMITER_SOPEN : TD_DELIMITER_PERIOD)) return FALSE;

	switch(id){
	case CMD_AXIS_SET_MINMAX:
		{
			double min_value, max_value;
			if(!pPaser->GetTokenDouble(&min_value)) return FALSE;
			if(!pPaser->TokenOut(TD_DELIMITER_COMMA)) return FALSE;
			if(!pPaser->GetTokenDouble(&max_value)) return FALSE;
			SetMinMax(min_value, max_value);
		}break;
	case CMD_AXIS_SET_ZOOMLIMIT:
		{
			double limit_value;
			if(!pPaser->GetTokenDouble(&limit_value)) return FALSE;
			SetZoomLimit(limit_value);
		}break;
	case CMD_AXIS_SET_AUTOMATIC:
		SetAutomatic(true);
		break;
	case CMD_AXIS_SET_AXISCOLOR:
		if(!m_AxisColor.Paser(pPaser)) return FALSE;
		break;
	case CMD_AXIS_SET_TEXTCOLOR:
		if(!m_TextColor.Paser(pPaser)) return FALSE;
		break;
	case CMD_AXIS_SET_INVERTED:
		if(!pPaser->GetTokenInt((int*)&m_bIsInverted)) return FALSE;
		RefreshScrollBar();
		break;
	case CMD_AXIS_LABEL:
		if(!GetLabel()->Paser(pPaser)) return FALSE;
		break;
	case CMD_AXIS_GRID:
		if(!GetGrid()->Paser(pPaser)) return FALSE;
		break;
	}

	if(id < CMD_AXIS_LABEL){
