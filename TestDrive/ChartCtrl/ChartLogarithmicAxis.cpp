...
typedef enum{
	CMD_LOGARITHMIC_AXIS_SET_AXISTOSCROLLSTEP,
	CMD_LOGARITHMIC_AXIS_SIZE,
}CMD_LOGARITHMIC_AXIS;

static const TCHAR* g_sChartLogarithmicAxis[CMD_LOGARITHMIC_AXIS_SIZE]={
	_T("SetAxisToScrollStep"),
};

BOOL CChartLogarithmicAxis::Paser(CPaser* pPaser){
	BOOL			bRet	= FALSE;
	int				iLoop	= 1;
	...
	if(pPaser->TokenOut(TD_DELIMITER_LOPEN)){
		while(iLoop){
			...
			case TD_TOKEN_NAME:
				{
					id = CheckCommand(token, g_sChartLogarithmicAxis, CMD_LOGARITHMIC_AXIS_SIZE);
					switch(id){
					case CMD_LOGARITHMIC_AXIS_SET_AXISTOSCROLLSTEP:
						{
							int iPrevStep, iCurStep;
							bool bSrcollInverted;
							if(!pPaser->GetTokenInt(&iPrevStep)) goto ERROR_OUT;
							if(pPaser->TokenOut(TD_DELIMITER_COMMA)) goto ERROR_OUT;
							if(!pPaser->GetTokenInt(&iCurStep)) goto ERROR_OUT;
							if(pPaser->TokenOut(TD_DELIMITER_COMMA)) goto ERROR_OUT;
							if(!pPaser->GetTokenInt((int*)&bSrcollInverted)) goto ERROR_OUT;
							SetAxisToScrollStep(iPrevStep, iCurStep, bSrcollInverted);
						}
						break;
					default:
