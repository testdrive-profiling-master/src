...
typedef enum{
	CMD_CHART_BARSERIE_SET_HORIZONTAL,
	CMD_CHART_BARSERIE_SET_BORDERCOLOR,
	CMD_CHART_BARSERIE_SET_BORDERWIDTH,
	CMD_CHART_BARSERIE_SET_BARWIDTH,
	CMD_CHART_BARSERIE_SHOW_GRADIENT,
	CMD_CHART_BARSERIE_SET_GRADIENT,
	CMD_CHART_BARSERIE_SET_STACKED,
	CMD_CHART_BARSERIE_SIZE
}CMD_CHART_BARSERIE;

static const TCHAR* g_sChartBarSerie[CMD_CHART_BARSERIE_SIZE]={
	_T("SetHorizontal"),
	_T("SetBorderColor"),
	_T("SetBorderWidth"),
	_T("SetBarWidth"),
...
BOOL CChartBarSerie::Paser(CPaser* pPaser){
	BOOL			bRet	= FALSE;
	int				iLoop	= 1;
	...
	if(pPaser->TokenOut(TD_DELIMITER_LOPEN)){
		while(iLoop){
			...
			case TD_TOKEN_NAME:
				{
					id = CheckCommand(token, g_sChartBarSerie, CMD_CHART_BARSERIE_SIZE);
					if(!pPaser->TokenOut(TD_DELIMITER_SOPEN)) goto ERROR_OUT;
					switch(id){
					case CMD_CHART_BARSERIE_SET_HORIZONTAL:
						{
							bool bHorizon;
							if(!pPaser->GetTokenInt((int*)&bHorizon)) goto ERROR_OUT;
							SetHorizontal(bHorizon);
						}
						break;
					case CMD_CHART_BARSERIE_SET_BORDERCOLOR:
						if(!m_BorderColor.Paser(pPaser)) goto ERROR_OUT;
						break;
					case CMD_CHART_BARSERIE_SET_BORDERWIDTH:
						if(!pPaser->GetTokenInt(&m_iBorderWidth)) goto ERROR_OUT;
						break;
					case CMD_CHART_BARSERIE_SET_BARWIDTH:
						if(!pPaser->GetTokenInt(&m_iBarWidth)) goto ERROR_OUT;
						break;
					case CMD_CHART_BARSERIE_SHOW_GRADIENT:
						if(!pPaser->GetTokenInt((int*)&m_bGradient)) goto ERROR_OUT;
						break;
					case CMD_CHART_BARSERIE_SET_GRADIENT:
						...
						break;
					case CMD_CHART_BARSERIE_SET_STACKED:
						if(!pPaser->GetTokenInt((int*)&m_bStacked)) goto ERROR_OUT;
						break;
					default:
