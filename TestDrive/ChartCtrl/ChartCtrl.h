...
typedef enum{	// refer to EGradientType
	CMD_GRADIENT_TYPE_HORIZONTAL,
	CMD_GRADIENT_TYPE_VERTICAL,
	CMD_GRADIENT_TYPE_HORIZONTALDOUBLE,
	CMD_GRADIENT_TYPE_VERTICALDOUBLE,
	CMD_GRADIENT_TYPE_SIZE
}CMD_GRADIENT_TYPE;

extern const TCHAR* g_sSidePosition[CMD_SIDE_SIZE];
extern const TCHAR* g_sGradientType[CMD_GRADIENT_TYPE_SIZE];
...
public:
	...
	**/
	...
	STDMETHOD_(ITDChartSerie*, CreateSerie)(CHART_STYLE style = CHART_STYLE_LINE, BOOL bSecondaryHorizAxis=FALSE, BOOL bSecondaryVertAxis=FALSE);
	...
	**/
	STDMETHOD_(ITDChartSerie*, GetSerie)(DWORD dwSerieID);		// get a serie
	//CChartSerie* GetSerie(unsigned dwSerieID) const;
	//! Removes a specific series from the chart
	/**
		@param uSerieId
			The Id of the series to be removed.
	**/
	STDMETHOD_(void, RemoveSerie)(DWORD dwSerieID);
	//! Removes all the series from the chart
	STDMETHOD_(void, RemoveAllSeries)(void);
	//! Returns the number of series in the chart
	STDMETHOD_(int, GetSeriesCount)(void);
...
private:
	...
	**/
	...
	CChartColor m_BackColor;	
	//! The border color
	CChartColor m_BorderColor;
