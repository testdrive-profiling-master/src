...
CChartAxisLabel::CChartAxisLabel()
  : m_pParentCtrl(NULL), m_bIsVisible(true),
...
void CChartAxisLabel::SetText(LPCTSTR NewText)  
{ 
...
typedef enum{
	CMD_AXIS_LABEL_SET_TEXT,
	CMD_AXIS_LABEL_SET_FONT,
	CMD_AXIS_LABEL_SET_COLOR,
	CMD_AXIS_LABEL_SIZE,
}CMD_AXIS_LABEL;

static const TCHAR* g_sChartAxisLabel[CMD_AXIS_LABEL_SIZE]={
	_T("SetText"),
	_T("SetFont"),
	_T("SetColor"),
};

BOOL CChartAxisLabel::Paser(CPaser* pPaser){
	if(!pPaser) return FALSE;
	TCHAR		token[MAX_PATH];
	int id;
	
	if(!pPaser->GetTokenName(token)) return FALSE;
	id = CheckCommand(token, g_sChartAxisLabel, CMD_AXIS_LABEL_SIZE);
	if(id >= CMD_AXIS_LABEL_SIZE) return FALSE;

	if(!pPaser->TokenOut(TD_DELIMITER_SOPEN)) return FALSE;

	switch(id){
	case CMD_AXIS_LABEL_SET_TEXT:
		{
			if(!pPaser->GetTokenString(token)) return FALSE;
			SetText(token);
		}break;
	case CMD_AXIS_LABEL_SET_FONT:
		{
			CChartFont font;
			if(!font.Paser(pPaser)) return FALSE;
			SetFont(font);
		}break;
	case CMD_AXIS_LABEL_SET_COLOR:
