...
void CChartCandlestickSerie::AddPointLevel(double XVal, 
									  double Low, 
									  double High, 
									  double Open, 
									  double Close)
...
void CChartCandlestickSerie::SetWidth(int iWidth)
{
	m_iCandlestickWidth = iWidth; 
...
typedef enum{
	CMD_CHART_CANDLESTICKSERIE_SET_WIDTH,
	CMD_CHART_CANDLESTICKSERIE_SIZE
}CMD_CHART_CANDLESTICKSERIE;

static const TCHAR* g_sChartCandlestickSerie[CMD_CHART_CANDLESTICKSERIE_SIZE]={
	_T("SetWidth"),
};

BOOL CChartCandlestickSerie::Paser(CPaser* pPaser){
	BOOL			bRet	= FALSE;
	int				iLoop	= 1;
	...
	if(pPaser->TokenOut(TD_DELIMITER_LOPEN)){
		while(iLoop){
			...
			case TD_TOKEN_NAME:
				{
					id = CheckCommand(token, g_sChartCandlestickSerie, CMD_CHART_CANDLESTICKSERIE_SIZE);
					if(!pPaser->TokenOut(TD_DELIMITER_SOPEN)) goto ERROR_OUT;
					switch(id){
					case CMD_CHART_CANDLESTICKSERIE_SET_WIDTH:
						if(!pPaser->GetTokenInt(&m_iCandlestickWidth)) goto ERROR_OUT;
						break;
					default:
