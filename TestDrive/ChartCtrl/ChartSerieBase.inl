...
template<class T>
void CChartSerieBase<T>::AddPointArray(T* pPoints, unsigned Count)
{
...
template<class T>
void CChartSerieBase<T>::Clear(void)
{
	m_vPoints.Clear();
	TLabelMap::iterator iter = m_mapLabels.begin();
	for (iter; iter!=m_mapLabels.end(); iter++)
...
template<class T>
void CChartSerieBase<T>::AddLabel(DWORD dwIndex, LPCTSTR lpszLabel){
	TChartString str = lpszLabel;
	CreateBalloonLabel(dwIndex, str);
	SetVisible(TRUE);
}
