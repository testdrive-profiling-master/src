...
typedef enum{
	CMD_STANDARD_AXIS_SET_TICKINCREMENT,
	CMD_STANDARD_AXIS_SIZE,
}CMD_STANDARD_AXIS;

static const TCHAR* g_sChartStandardAxis[CMD_STANDARD_AXIS_SIZE]={
	_T("SetTickIncrement"),
};

BOOL CChartStandardAxis::Paser(CPaser* pPaser){
	BOOL			bRet	= FALSE;
	int				iLoop	= 1;
	...
	if(pPaser->TokenOut(TD_DELIMITER_LOPEN)){
		while(iLoop){
			...
			case TD_TOKEN_NAME:
				{
					id = CheckCommand(token, g_sChartStandardAxis, CMD_STANDARD_AXIS_SIZE);
					switch(id){
					case CMD_STANDARD_AXIS_SET_TICKINCREMENT:
						{
							bool bAuto;
							double increment;
							if(!pPaser->GetTokenInt((int*)&bAuto)) goto ERROR_OUT;
							if(pPaser->TokenOut(TD_DELIMITER_COMMA)) goto ERROR_OUT;
							if(!pPaser->GetTokenDouble(&increment)) goto ERROR_OUT;
