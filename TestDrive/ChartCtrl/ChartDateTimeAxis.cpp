...
typedef enum{
	CMD_DATETIME_AXIS_SET_TICKINCREMENT,
	CMD_DATETIME_AXIS_SET_TICKLABELFORMAT,
	CMD_DATETIME_AXIS_SIZE,
}CMD_DATETIME_AXIS;

static const TCHAR* g_sChartDateTimeAxis[CMD_DATETIME_AXIS_SIZE]={
	_T("SetTickIncrement"),
	_T("SetTickLabelFormat"),
};

typedef enum{	// refer to TimeInterval
	CMD_TIMEINETERVAL_TYPE_SECOND,
	CMD_TIMEINETERVAL_TYPE_MINUTE,
	CMD_TIMEINETERVAL_TYPE_HOUR,
	CMD_TIMEINETERVAL_TYPE_DAY,
	CMD_TIMEINETERVAL_TYPE_MONTH,
	CMD_TIMEINETERVAL_TYPE_YEAR,
	CMD_TIMEINETERVAL_TYPE_SIZE,
}CMD_TIMEINETERVAL_TYPE;

static const TCHAR* g_sTimeIntervalType[CMD_TIMEINETERVAL_TYPE_SIZE]={
	_T("Second"),
	_T("Minute"),
	_T("Hour"),
	_T("Day"),
...
BOOL CChartDateTimeAxis::Paser(CPaser* pPaser){
	BOOL			bRet	= FALSE;
	int				iLoop	= 1;
	...
	if(pPaser->TokenOut(TD_DELIMITER_LOPEN)){
		while(iLoop){
			...
			case TD_TOKEN_NAME:
				{
					id = CheckCommand(token, g_sChartDateTimeAxis, CMD_DATETIME_AXIS_SIZE);
					switch(id){
					case CMD_DATETIME_AXIS_SET_TICKINCREMENT:
						if(!pPaser->GetTokenInt((int*)&m_bAutoTicks)) goto ERROR_OUT;
						if(!m_bAutoTicks){
							if(!pPaser->GetTokenName(token)) goto ERROR_OUT;
							m_BaseInterval = (TimeInterval)CheckCommand(token, g_sTimeIntervalType, CMD_TIMEINETERVAL_TYPE_SIZE);
							if(m_BaseInterval >= CMD_TIMEINETERVAL_TYPE_SIZE) goto ERROR_OUT;
							if(!pPaser->GetTokenInt(&m_iDTTickIntervalMult)) goto ERROR_OUT;
						}
						break;
					default:
						if(!CChartAxis::Paser(token, pPaser)) return FALSE;
					}
				}break;
