...
BOOL CChartFont::Paser(CPaser* pPaser){
	if(!pPaser) return FALSE;
	{
		...
		// font size
		if(!pPaser->GetTokenInt(&m_iPointSize)) return FALSE;
		if(pPaser->TokenOut(TD_DELIMITER_COMMA)){
			// bold
			if(!pPaser->GetTokenInt((int*)&m_bBold)) return FALSE;
			if(pPaser->TokenOut(TD_DELIMITER_COMMA)){
				// italic
				if(!pPaser->GetTokenInt((int*)&m_bItalic)) return FALSE;
				if(pPaser->TokenOut(TD_DELIMITER_COMMA)){
					// underline
					if(!pPaser->GetTokenInt((int*)&m_bUnderline)) return FALSE;
					if(pPaser->TokenOut(TD_DELIMITER_COMMA)){
						// vertical
						if(!pPaser->GetTokenInt((int*)&m_bVertical)) return FALSE;
					}
				}
			}
		}
