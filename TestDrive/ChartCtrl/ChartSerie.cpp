...
//	m_uLastDrawnPoint = 0;
	...
	m_uSerieId = pParent->m_uNextFreeSerieId;
	pParent->m_uNextFreeSerieId++;
...
void CChartSerie::SetName(LPCTSTR NewName) 
{ 
	m_strSerieName = NewName; 
	m_pParentCtrl->RefreshCtrl();
}

double CChartSerie::XScreenToValue(long XScreenCoord)
{
	return m_pHorizontalAxis->ScreenToValue(XScreenCoord);
}

double CChartSerie::YScreenToValue(long YScreenCoord)
{
...
typedef enum{
	CMD_CHART_SERIE_SET_NAME,
	CMD_CHART_SERIE_SET_COLOR,
	CMD_CHART_SERIE_SET_SHADOW_COLOR,
	CMD_CHART_SERIE_ENABLE_SHADOW,
	CMD_CHART_SERIE_SET_SHADOW_DEPTH,
	CMD_CHART_SERIE_SIZE
}CMD_CHART_SERIE;

const TCHAR* g_sChartSerie[CMD_CHART_SERIE_SIZE]={
	_T("SetName"),
	_T("SetColor"),
	_T("SetShadowColor"),
	_T("EnableShadow"),
...
BOOL CChartSerie::Paser(LPCTSTR pCmd, CPaser* pPaser){
	if(!pCmd || !pPaser) return FALSE;
	TCHAR			token[MAX_PATH];
	int				id;
	id = CheckCommand(pCmd, g_sChartSerie, CMD_CHART_SERIE_SIZE);

	switch(id){
	case CMD_CHART_SERIE_SET_NAME:
		if(!pPaser->GetTokenString(token)) return FALSE;
		m_strSerieName	= token;
		break;
	case CMD_CHART_SERIE_SET_COLOR:
		if(!m_SerieColor.Paser(pPaser)) return FALSE;
		break;
	case CMD_CHART_SERIE_SET_SHADOW_COLOR:
		if(!m_ShadowColor.Paser(pPaser)) return FALSE;
		break;
	case CMD_CHART_SERIE_ENABLE_SHADOW:
		if(!pPaser->GetTokenInt((int*)&m_bShadow)) return FALSE;
		break;
	case CMD_CHART_SERIE_SET_SHADOW_DEPTH:
		if(!pPaser->GetTokenInt(&m_iShadowDepth)) return FALSE;
		break;
