...
void CChartLineSerie::SetWidth(int iWidth)  
{ 
	m_iLineWidth = iWidth; 
	m_pParentCtrl->RefreshCtrl();
}
...
typedef enum{
	CMD_CHART_LINESERIE_SET_WIDTH,
	CMD_CHART_LINESERIE_SET_SMOOTH,
	CMD_CHART_LINESERIE_SET_PENSTYLE,
	CMD_CHART_LINESERIE_SIZE
}CMD_CHART_LINESERIE;

const TCHAR* g_sChartLineSerie[CMD_CHART_LINESERIE_SIZE]={
	_T("SetWidth"),
	_T("SetSmooth"),
	_T("SetPenStyle"),
};
...
typedef enum{
	...
	CMD_PENSTYLE_DASH,
	CMD_PENSTYLE_DOT,
	CMD_PENSTYLE_DASHDOT,
	CMD_PENSTYLE_DASHDOTDOT,
	CMD_PENSTYLE_SIZE,
}CMD_PENSTYLE;

const TCHAR* g_sPenStyle[CMD_PENSTYLE_SIZE]={
	_T("Solid"),
	_T("Dash"),
	_T("Dot"),
	_T("DashDot"),
...
BOOL CChartLineSerie::Paser(CPaser* pPaser){
	BOOL			bRet	= FALSE;
	int				iLoop	= 1;
	...
	if(pPaser)
		if(pPaser->TokenOut(TD_DELIMITER_LOPEN)){
			while(iLoop){
				...
				case TD_TOKEN_NAME:
					{
						id = CheckCommand(token, g_sChartLineSerie, CMD_CHART_LINESERIE_SIZE);
						if(!pPaser->TokenOut(TD_DELIMITER_SOPEN)) goto ERROR_OUT;
						switch(id){
						case CMD_CHART_LINESERIE_SET_WIDTH:
							if(!pPaser->GetTokenInt(&m_iLineWidth)) goto ERROR_OUT;
							break;
						case CMD_CHART_LINESERIE_SET_SMOOTH:
							if(!pPaser->GetTokenInt((int*)&m_bSmooth)) goto ERROR_OUT;
							break;
						case CMD_CHART_LINESERIE_SET_PENSTYLE:
							if(!pPaser->GetTokenName(token)) goto ERROR_OUT;
							m_iPenStyle = CheckCommand(token, g_sPenStyle, CMD_PENSTYLE_SIZE);
							if(m_iPenStyle >= CMD_PENSTYLE_SIZE) goto ERROR_OUT;
							break;
						default:
