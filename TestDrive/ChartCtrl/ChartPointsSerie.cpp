...
typedef enum{
	CMD_CHART_POINTSERIE_SET_POINTSIZE,
	CMD_CHART_POINTSERIE_SET_POINTTYPE,
	CMD_CHART_POINTSERIE_SET_BORDERCOLOR,
	CMD_CHART_POINTSERIE_SIZE
}CMD_CHART_POINTSERIE;

const TCHAR* g_sChartPointSerie[CMD_CHART_POINTSERIE_SIZE]={
	_T("SetPointSize"),
	_T("SetPointType"),
	_T("SetBorderColor"),
};
...
typedef enum{
	CMD_POINT_TYPE_ELLIPSE,
	CMD_POINT_TYPE_RECTANGLE,
	CMD_POINT_TYPE_TRIANGLE,
	CMD_POINT_TYPE_SIZE
}CMD_POINT_TYPE;

const TCHAR* g_sPointType[CMD_POINT_TYPE_SIZE]={
	_T("Ellipse"),
	_T("Rectangle"),
	_T("Triangle"),
};

BOOL CChartPointsSerie::Paser(CPaser* pPaser){
	BOOL			bRet	= FALSE;
	int				iLoop	= 1;
	...
	if(pPaser->TokenOut(TD_DELIMITER_LOPEN)){
		while(iLoop){
			...
			case TD_TOKEN_NAME:
				{
					id = CheckCommand(token, g_sChartPointSerie, CMD_CHART_POINTSERIE_SIZE);
					if(!pPaser->TokenOut(TD_DELIMITER_SOPEN)) goto ERROR_OUT;
					switch(id){
					case CMD_CHART_POINTSERIE_SET_POINTSIZE:
						if(!pPaser->GetTokenInt(&m_iXPointSize)) goto ERROR_OUT;
						if(!pPaser->TokenOut(TD_DELIMITER_COMMA)) goto ERROR_OUT;
						if(!pPaser->GetTokenInt(&m_iYPointSize)) goto ERROR_OUT;
						break;
					case CMD_CHART_POINTSERIE_SET_POINTTYPE:
						if(!pPaser->GetTokenName(token)) goto ERROR_OUT;
						m_iPointType = (PointType)CheckCommand(token, g_sPointType, CMD_POINT_TYPE_SIZE);
						if(m_iPointType >= CMD_POINT_TYPE_SIZE) goto ERROR_OUT;
						break;
					case CMD_CHART_POINTSERIE_SET_BORDERCOLOR:
						if(!m_colBorder.Paser(pPaser)) goto ERROR_OUT;
						break;
					default:
